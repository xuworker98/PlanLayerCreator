...
class LineTab(BaseTab):
    def __init__(self, parent=None):
        self.req_fields = [
            ('type_col', '唯一标识', True),
            ('wkt_col', 'WKT几何列', True),
        ]
        super().__init__('line', self.req_fields, parent)

    def get_default_style(self):
        return {'line_color': '#0984e3', 'line_width': 2.0}
    ...
    def _build_style_section(self):
        ...
        p1 = QHBoxLayout(); p1.setSpacing(2); p1.addWidget(QLabel("线条颜色：")); p1.addWidget(self.line_color_btn)
        row.addLayout(p1)
        sp = QLabel(" "); sp.setFixedWidth(16); row.addWidget(sp)

        self.line_width_spin = QDoubleSpinBox()
        self.line_width_spin.setRange(0.5, 10.0); self.line_width_spin.setValue(self.style['line_width'])
        self.line_width_spin.setMaximumWidth(80)
        p2 = QHBoxLayout(); p2.setSpacing(2); p2.addWidget(QLabel("线条宽度：")); p2.addWidget(self.line_width_spin)
        row.addLayout(p2)
        row.addStretch()
    ...
    def pick_line_color(self):
        c = QColorDialog.getColor(QColor(self.style['line_color']), self, "线条颜色")
        if c.isValid():
    ...
    def get_extra_params(self):
        return {'line_color': self.style['line_color'], 'line_width': self.line_width_spin.value()}
    ...
    def generate(self):
        ...
        self._on_gen_start()
        self._worker = GenerateWorker('line', self.df, mapping, self.style, output_path, do_correct, self.get_extra_params())
    ...
    def get_template_data(self):
        return {
            '序号': [1, 2], '类型': ['LINESTRING', 'LINESTRING'],
            'WKT': ['LINESTRING (108.3718 22.8715, 108.3738 22.8693)',
                     'LINESTRING (108.3738 22.8693, 108.3752 22.8680)'],
        }
